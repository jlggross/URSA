...
public class OldFullStarsClusteringStrategy extends ClusteringStrategy {
	private double threshold;
	
	public OldFullStarsClusteringStrategy(double threshold) {
	...
	public List<DataCluster> executeClustering(List<DataObject> dataObjects, Matrix2D similarityMatrix) {
		...
		for (int i = 0; i < totalDocs; i++) {
			...
			//if (!allocatedObjects.contains(currentObject)) {
				// IGNORE - The current object is not allocated yet, so starts a new cluster.
